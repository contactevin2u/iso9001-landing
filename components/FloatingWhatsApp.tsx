...
export default function FloatingWhatsApp({ locale = 'en' }: { locale?: Locale }) {
  ...
  return (
    ...
    >
      ...
      {/* Icon + text container */}
      <span className="relative flex items-center gap-2 px-4 py-3 sm:px-5 sm:py-3.5">
        <svg className="w-6 h-6 sm:w-7 sm:h-7 flex-shrink-0" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
        ...
        <span className="text-sm sm:text-base font-semibold whitespace-nowrap">{t.floatingLabel}</span>
      </span>
    </a>
  )
}
